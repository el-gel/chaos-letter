...
Runs games as multiple rounds. Config options to set number of hearts / insanity tokens for a win.
...
# Turn starts
Player A's turn starts. This puts a turn started Event on the queue; this is seen by Player A's Handmaid, which adds a remove protection Event conditional on the starting of the turn. Nothing else can respond, so the protection is removed by the Handmaid's effect. (The Handmaid does this by using the Card 'on_event' trigger.)
...
The top card of the deck is taken away, and the card will be asked to run its 'on_discard' effect. In this case, it's sane, so nothing happens. If nothing in the deck, then nothing happens.
...
Player A is immortal, and the immortal card sees the player dies Event and cancels it, by queuing a "no death" Event which is tied to the player dies Event.
The queue now has a 'player dies' and 'no death' event. The no death one fires, and then resolves since nothing responds. It cancels the 'player dies' Event.
...
To form this Query, the Game asks each of the cards in their hand about what play options they have (We don't yet have a 'play from discard or another player's hand', so only checking those two cards).
...
Then the play Event fires. This is shown to each player (priority should be to targeted players, then non-targeted players, in turn order from the user. Not currently implemented).
...
Game objects to consider, and links between (ignoring links that are Hidden):
- Card
